...
function ForgetPasswordUser() {
    ...
    useEffect(() => {
        ...
        if (Object.keys(errors).length === 0 && submitted) {
            console.log(user)
            axios.post("http://localhost:8081/RentAndRide/user/forgetpassword", user,config)
                .then(resp => {
                    ...
                    sessionStorage.setItem("email", result.email)
                    setUser({ ...user, generatedOtp: result.sendOtp })
                    console.log(user);
                    setSubmitted(false)
                })
                .catch(error => {
    ...
    return (
        <div className="container-fluid w-50 mt-5 login-component" >
            <div
                className="row shadow bg-transparent border rounded"
            >
                <div className="col-4 ">
                    <img
                        // src={user1}
                        ...
                        <div className="">
                            <form onSubmit={handleSubmit}>
                                <div className="form-floating mb-3">
                                    <label><i className="fa fa-envelope pr-2"></i></label>
                                    Email 
                                    <input
                                        type="text"
                                        name="email"
